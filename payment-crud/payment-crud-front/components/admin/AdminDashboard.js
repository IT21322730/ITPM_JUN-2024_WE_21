...
const Sidebar = () => {
  return (
    <div style={{ width: '250px', backgroundColor: '#FFA500', color: 'black', padding: '10px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'flex-end', height: '100vh' }}>
      {/* <img src={Logo} alt="Logo" style={{ marginBottom: '20px' }} /> */}
      <SidebarItem icon={<Dashboard fontSize="large" />} name="Dashboard" link="/admin/complain" />
      <SidebarItem icon={<ShoppingCart fontSize="large" />} name="Order" link="/admin/order" />
      <SidebarItem icon={<LocalMall fontSize="large" />} name="Items" link="/allItems" />
      ...
      <SidebarItem icon={<Restaurant fontSize="large" />} name="Restaurant" link="/admin/restaurants" />
      <SidebarItem icon={<LocalShipping fontSize="large" />} name="Delivery" link="/admin/delivery" />
      <SidebarItem icon={<Campaign fontSize="large" />} name="Complain" link="/admin/promotion" />
    </div>
  );
}
