...
const Korean = () => {
    ...
    const [koreanItems, setKoreanItems] = useState([]);
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                ...
                const filteredItems = response.data.filter(item => item.itemCategory === 'Korean');
                setKoreanItems(filteredItems);
            } catch (error) {
    ...
    return (
        <div style={{ textAlign: 'center' }}>
            <Header />
            <h2 style={{ marginBottom: '20px' }}>Korean Cuisine</h2>
