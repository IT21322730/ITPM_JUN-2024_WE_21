...
const Japanese = () => {
    ...
    const [japaneseItems, setJapaneseItems] = useState([]);
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                ...
                const filteredItems = response.data.filter(item => item.itemCategory === 'Japanese');
                setJapaneseItems(filteredItems);
            } catch (error) {
    ...
    return (
        <div style={{ textAlign: 'center' }}>
            <Header />
            <h2 style={{ marginBottom: '20px' }}>Japanese Cuisine</h2>
            <div style={{ display: 'flex', justifyContent: 'center' }}>
                {japaneseItems.map((item, index) => (
