...
const Thai = () => {
    ...
    const [thaiItems, setThaiItems] = useState([]);
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                ...
                const filteredItems = response.data.filter(item => item.itemCategory === 'Thai');
                setThaiItems(filteredItems);
            } catch (error) {
    ...
    return (
        <div style={{ textAlign: 'center' }}>
            <Header/>
            <h2 style={{ marginBottom: '20px' }}>Thai Cuisine</h2>
